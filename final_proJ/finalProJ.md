...
* 특정 사람에 대해서 검색어가 어떤 것이었으면 좋겠습니다.
* 특정 사람에 대한 검색어에 대한 단위를 넣고, 분류에 대한 단위를 넣어야 함. -> 분류를 먼저 만들어야함.
* 초기 가입자, 이력이 있는 사람에 대해서 둘다 만들어야함.

## 데이터를 어떻게 다뤄야 하는가?

* 엄청 지저분한 데이터 형식으로 이루어짐.
* 가공을 해야함.
* 해당하는 부분에 대해서 분류를 만들어내야함.
* 사람 중심의 제안과 데이터 중심의 제안을 만들어야 함.
...
테이블과 테이블은 무조건 관계를 형성하게 되어 있음.

관계에 의해서 테이블이 형성됨. -> 이유는 중복을 피하기 위함.
...
1. pt : 챕터 수정 가능
   * 팀원 소개, 주제 선택 동기, 시스템 흐름도
   * 동작 영상 링크 : 유튜브
     * 프로젝트 용으로, 스크래핑하는 장면, 디버깅의 일부만 보여줬으면, 값을 확인하는 장면까지 나오도록(장고를 보여주기엔 너무 복잡함.)
   * 주요화면, 주요코드
   * __진행시 에피소드와 느낀 점.(불변)__ 불편했던 점! 개선 점
   * 제 3자가 봤을때, 이해가 되게끔 해야함. 
2. 깃
   1. main : 팀원 1줄 설명, 각 sub링크, 시스템 흐름도, 동작 이미지와 영상링크 : 유튜브
   2. sub 1 : 팀원 구성 혹은 역할, 프로젝트 성격(동기와 기간), 에피소드 및 느낀 점
   3. sub 2 : Code 구조 (아키텍처)
